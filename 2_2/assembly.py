...
class DSU:
    ...
    def is_merged(self, x, y) -> bool:
        return self.get_class(x) == self.get_class(y)

    def merge_classes(self, x, y) -> int:
...
def build_graph(reads: List[str]) -> Graph:
    graph = Graph()
    for read in reads:
...
def compute_assembly(reads: List[str]):
    # O(n^2 * l)
    graph = build_graph(reads)
    # Summary O(E log E + E * l) = O(n^2 log n + n^2 * l)
...
def test(input):
    ...
    print(f"Test output: {compute_assembly(input)}")
    print()


if __name__ == '__main__':
    test(["AAA", "AAT", "ATT", "TTA", "TTT"])
    test(["AAA", "CTA", "CCC", "TTA"])
