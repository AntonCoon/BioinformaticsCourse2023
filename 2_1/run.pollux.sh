...
# run the command
res1=$(date +%s)
$toolPath -i $input1 -o $outdir -k $kmer -n false -d false -h false >> $logfile 2>&1
res2=$(date +%s)
...
#
#cat $outdir/${basename1}.corrected | gzip > $outdir/${toolName}_$(basename ${input1%.*})_${kmer}.corrected.fastq.gz
